...
//            }
        ...
        if (self.playerViewType == XMPlayerViewTwoSynVideoType) {
            ...
            if (playerItem == self.playerItem) {
                NSLog(@"当前缓存时间视频1:%@",[NSString stringWithFormat:@"%.0f", totalBuffer]);
                self.playBufferTime1 = [[NSString stringWithFormat:@"%.0f", totalBuffer] intValue];
            }else if (playerItem == self.playerItem2){
                
                NSLog(@"当前缓存时间视频2:%@",[NSString stringWithFormat:@"%.0f", totalBuffer]);
            ...
            NSLog(@"当前缓存时间视频最小值:%d",self.minBufferTime);
...
//    NSLog(@"前播放时间视频2:%@",[NSString stringWithFormat:@"%.0f", currentPlayTime]);
}
