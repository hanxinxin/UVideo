...
- (instancetype)initWithContents:(NSArray <UIView *> *)contents andTapAction:(BOOL)tapFlag
{
    self = [super init];
    if (self)
    {
        ...
        {
            UIView *view = contents[0];
            if (CNP_IS_IPAD)
            {
                        topframe = CGRectMake(self.applicationWindow.center.x-250, self.applicationWindow.center.y-200, view.bounds.size.width, view.bounds.size.height);
            }
            else
            {
                        topframe = CGRectMake(self.applicationWindow.center.x-200, self.applicationWindow.center.y-150, view.bounds.size.width, view.bounds.size.height);
            }

            self.maskView = [[UIView alloc] initWithFrame:topframe];
        }
    
        NSLog(@"maskView center = %@",NSStringFromCGPoint(self.maskView.center));
...
- (instancetype)init {
    self = [self initWithContents:@[] andTapAction:YES];
    return self;
}
...
- (CGSize)calculateContentSizeThatFits:(CGSize)size andUpdateLayout:(BOOL)update
{
    ...
    if (update)
    {
        ...
        {
            if ([self.popupView.subviews count] == 1) {
                UIView *view = self.popupView.subviews[0];
                view.frame = CGRectMake(0, 0, result.width, result.height);
            }
            else
            {
                for (UIView *view in self.popupView.subviews)
...
- (void)presentPopupControllerAnimated:(BOOL)flag {
   ...
   // NSLog(@"top = %@,self.maskView = %@,self.popupView.frame = %@",NSStringFromCGRect(topframe),NSStringFromCGRect(self.maskView.frame),NSStringFromCGRect(self.popupView.frame));
    ...
    [UIView animateWithDuration:flag?self.theme.animationDuration:0.0 animations:^{
        ...
        {
           // NSLog(@"top = %@,self.maskView = %@,self.popupView.frame = %@",NSStringFromCGRect(topframe),NSStringFromCGRect(self.maskView.frame),NSStringFromCGRect(self.popupView.frame));
            UIView *backview = self.popupView.subviews[0];
           //  NSLog(@"view = %@",NSStringFromCGRect(backview.frame));
        }
