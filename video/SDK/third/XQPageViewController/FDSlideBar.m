...
//    gl.frame = CGRectMake(26,98,16,2);
//    gl.startPoint = CGPointMake(0, 1);
//    gl.endPoint = CGPointMake(1, 0);
//    gl.colors = @[(__bridge id)RGB(209,238,255).CGColor,(__bridge id)RGB(20,155,236).CGColor];
//    gl.locations = @[@(0),@(1.0f)];
//    [self.sliderView.layer addSublayer:gl];
...
- (void)setupItems {
    ...
    _sliderView.frame = CGRectMake(0, self.frame.size.height - SLIDER_VIEW_HEIGHT-3, firstItem.frame.size.width/4, SLIDER_VIEW_HEIGHT);
...
- (void)scrollToVisibleItem:(FDSlideBarItem *)item {
    dispatch_async(dispatch_get_main_queue(), ^{
        [UIView animateWithDuration:0.25 delay:0 options:UIViewAnimationOptionCurveLinear | UIViewAnimationOptionBeginFromCurrentState animations:^{
            ...
            self->_sliderView.frame = CGRectMake(CGRectGetMinX(item.frame)+(CGRectGetWidth(item.frame)/2)/2, self->_sliderView.frame.origin.y, CGRectGetWidth(item.frame)/2, SLIDER_VIEW_HEIGHT);
